package net.Teepi_.mccoursemod.item.custom;

import com.google.common.collect.ImmutableMap;
import net.Teepi_.mccoursemod.item.ModArmorMaterials;
...
//    @Override
//    public void onArmorTick(ItemStack stack, Level level, Player player, int ) {
//        if(!level.isClientSide() && hasFullSuitOfArmorOn(player)) {
//            evaluateArmorEffects(player);
//        }
//    }
    ...
    public void inventoryTick(ItemStack stack, Level level, Entity pEntity, int pSlotId, boolean pIsSelected) {
        if (!level.isClientSide) {
            if (pEntity instanceof Player player) {
                if (hasFullSuitOfArmorOn(player) && player.getHealth() < (player.getMaxHealth() / 2)) {
                    evaluateArmorEffects(player);
                }
